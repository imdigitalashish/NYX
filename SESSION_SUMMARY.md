...
## The headline
Nyx wins on Gemini by exploiting an overlooked discovery: **Gemini bills image
tokens FLAT (~1080) for any size up to 25 Mpx.** Pack the whole doc into wide flat-billed
pages -> ~35% avg (50% on large docs) fewer tokens than the narrow baseline, ~86% fewer than text, at
equal-or-better accuracy on real content.

## Biggest surprises
1. **Gemini reads 40-char SHA-1 hashes 8/8 EXACT** — the field's "optical is lossy for
   verbatim" assumption (weak-encoder 0/15, DeepSeek-OCR 60%@20x) is a weak-encoder artifact.
